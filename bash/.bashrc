...
_jarvis_greeting() {
    ...
    if [ "$JARVIS_MODE" = "FRIDAY" ]; then
        ...
        printf "  ╠%s╣\n" "$sep"
        printf "  ║%s║\n" "$(_brow "  Hey, $(whoami). Good $period." "$interior")"
        printf "  ║%s║\n" "$(_brow "  $datetime" "$interior")"
        [ -n "$uptime_str" ] && printf "  ║%s║\n" "$(_brow "  Uptime: $uptime_str" "$interior")"
        [ -n "$mem_info" ]   && printf "  ║%s║\n" "$(_brow "  Memory: $mem_info   CPU: $cpu_load" "$interior")"
        ...
        printf "  ╠%s╣\n" "$sep"
        printf "  ║%s║\n" "$(_brow "  Good $period, $(whoami)." "$interior")"
        printf "  ║%s║\n" "$(_brow "  $datetime" "$interior")"
        [ -n "$uptime_str" ] && printf "  ║%s║\n" "$(_brow "  Uptime: $uptime_str" "$interior")"
        [ -n "$mem_info" ]   && printf "  ║%s║\n" "$(_brow "  Memory: $mem_info   CPU: $cpu_load" "$interior")"
        [ -n "$container" ]  && printf "  ║%s║\n" "$(_brow "  Container: $container" "$interior")"
